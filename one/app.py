...
OS_VERSION = "1.6.0"   # Downstage OS release - bump on tagged releases
...
def _health_summary():
    ...
    try:
        if load_config().get("mode", "local") == "local" and not ontime_is_running():
            probs.append("OnTime stopped")
    except Exception:
        pass
    try:
        hc = hdmi_connected()
        missing = [n for n in ("1", "2") if not hc.get(n)]
        if missing:
            probs.append("HDMI " + "+".join(missing) + " no display")
    except Exception:
        pass
    try:
        t = _cpu_temp()
...
def network_apply():
    ...
    if iface_q in ("eth0", "wlan0"):
        ...
        conn, iface = _default_conn()
    if not conn:
        return jsonify({"ok": False, "message": "No profile found for that interface"})
...
.wm { font-size:min(5.5vh, 6vw); font-weight:700; letter-spacing:0.1em; color:#E8ECEF; }
.wm b { color:#2FD97B; }
